...
- **Block 0 unreliable**: Block 0 does not retain programmed data on some W25N01GV units. Use a write offset of at least `0x20000`.
- **No bad-block skip in `read-flash` / `verify-flash`**: These commands use raw physical addresses. If `write-flash` skipped bad blocks, use the actual write address printed during the write operation.
- **Read size must match write size**: NAND pads writes to full 128 KB blocks. Reading back a different size may produce byte-for-byte mismatches at the end.
