...
def main():
    ...
    with open(args.base_stub_elf, 'rb') as f:
        ...
        # Base-stub functions that plugins call by absolute address. Plugin
        # linker scripts reference them as BASE_<UPPER_NAME>; resolve each to
        # its address in the base stub so the plugin link can --defsym it.
        symtab = elf.get_section_by_name('.symtab')
        base_syms = {}
        if symtab is not None:
            for sym in symtab.iter_symbols():
                if sym.name:
                    base_syms[sym.name] = sym['st_value']
        base_defsyms = []
        for name in ('slip_send_frame',):
            addr = base_syms.get(name)
            if addr is not None:
                base_defsyms.append('-Wl,--defsym,BASE_%s=0x%08X' % (name.upper(), addr))
    ...
    if base_defsyms:
        lines.append('set(STUB_BASE_DEFSYMS %s)' % ' '.join('"%s"' % d for d in base_defsyms))
